...
def load_data(file_paths, selected_features, date, run_id):
    selected_file_paths = []
...
def load_data_create_images(cfg, load_only = False):
    ...
    df_time = load_data(file_paths, selected_features, date, run_id)
    ...
    for attack in tqdm(selected_attacks):
        ...
        if load_only == False:
            ...
            if run_type == 'unit':
                index = index[0:num_of_samples]
            dataset_dict[attack] = {"x_data" : x_data[index], "y_data" : y_data[index]}
        else:
            index = np.arange(df_attack.shape[0])
            if run_type == 'unit':
                index = index[0:num_of_samples]
            dataset_dict[attack] = df_attack.iloc[index]
    return dataset_dict