...
def wgan_evaluate_pipeline(cfg: DictConfig) -> None:
    ...
    if cfg.models.model_type != 'baselines':
        ...
        for window in cfg.list_of_windows:
        ...
        if found_wgan:
            ...
            # Directories...
            dir_perf_gen_dis = cfg.workspace_dir / 'artifacts' / f'results_{version}' / f'performance_gen_dis.csv'
            dir_corr_gen_dis =  cfg.workspace_dir / 'artifacts' / f'results_{version}' / f'correlation_gen_dis.csv'
            # Save data..
...
python run_ind_evaluation_pipeline.py version=october_24_wisec_10 results_only=false dataset=testing
python run_ind_evaluation_pipeline.py version=october_24_wisec_ae models=wgan list_of_windows=[10] results_only=False dataset=testing dataset.run_type=unit
nohup python run_ind_evaluation_pipeline.py version=november_8_wgan_8 models=wgan list_of_windows=[8] results_only=False dataset=testing >/dev/null 2>&1 &
nohup python run_ind_evaluation_pipeline.py version=october_24_wisec results_only=False dataset=testing >/dev/null 2>&1 &
"""