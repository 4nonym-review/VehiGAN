...
def generate_results(cfg: DictConfig) -> None:
    ...
    ens_file_name = cfg.workspace_dir / 'artifacts' / f'results_{version}' / f"ens_detector_ads.csv"
    ensemmble_df.to_csv(ens_file_name)
    print("ensemmble_df :", ensemmble_df)

    max_file_name = cfg.workspace_dir / 'artifacts' / f'results_{version}' / f"max_detector.csv"
    ...
    # TODO: Save data and visualize
    ensemmble_df = pd.DataFrame(eval_dict).T
    ens_file_name = cfg.workspace_dir / 'artifacts' / f'results_{version}' / f"ens_detector_ags.csv"
    ensemmble_df.to_csv(ens_file_name)
...
"""
python run_ens_evaluation_pipeline.py version=november_4_wgan_10 dataset=testing list_of_windows=[10]
"""